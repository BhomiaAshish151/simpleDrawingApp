...
- (id) initWithNibName: (NSString*) nibNameOrNil bundle: (NSBundle*) nibBundleOrNil
{
	...
	if( self ) {
		self.navigationItem.title = NSLocalizedString( @"Choose Color", 
...
- (void) presentModallyOverViewController: (UIViewController*) controller
{
    ...
    UIImage *image = [UIImage imageNamed:@"addBtn.png"];
    UIButton* requestButton = [UIButton buttonWithType:UIButtonTypeCustom];
    
    requestButton.frame = CGRectMake(0, 0, 55, 35);
    
    [requestButton setBackgroundImage:image forState:UIControlStateNormal];
     [requestButton setTitle:@"Done" forState:UIControlStateNormal];
    
    [requestButton addTarget:self action:@selector(done:)
